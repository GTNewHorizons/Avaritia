...
public class Mincer {
    ...
    private static final String[] sacredCropNames = new String[] { "cropWheat", "cropCarrot", "cropPotato", "cropApple",
            "cropMelon", "cropPumpkin", "cropCactus", "cropMushroomRed", "cropMushroomBrown", "cropCherry" };
    private static final String[] forbiddenCropNames = new String[] { "cropEdibleroot", "cropWhitemushroom", "cropBeet",
            "cropCotton", "cropPoppy", "cropTulipRed", "cropTulipWhite", "cropDaisy", "cropTulipPink", "cropAllium",
            "cropOrchid", "cropTulipOrange", "cropDandelion", "cropShroomRed", "cropShroomBrown", "cropFerranium",
            "cropAurigold", "cropDiamahlia", "cropLapender", "cropEmeryllis", "cropRedstodendron", "cropCuprosia",
            "cropPetinia", "cropPlombean", "cropSilverweed", "cropJaslumine", "cropNiccissus", "cropPlatiolus",
    ...
    static {
        ...
        /*
         * for (int i=0; i<ItemFishFood.FishType.values().length; i++) { knownMeats.add(new ItemStack(Items.fish, 1,
         * i)); }
         */
    ...
    public static void countThoseCalories() {
        ...
        if (Config.boringFood) {
            stewRecipe = ExtremeCraftingManager.getInstance().addShapelessOreRecipe(
                    new ItemStack(LudicrousItems.ultimate_stew, 1),
            ...
            meatballRecipe = ExtremeCraftingManager.getInstance().addShapelessOreRecipe(
                    new ItemStack(LudicrousItems.cosmic_meatballs, 1),
        ...
        stewRecipe = ExtremeCraftingManager.getInstance().addShapelessOreRecipe(
                new ItemStack(LudicrousItems.ultimate_stew, makesstew),
        ...
        /*
         * for (int i=0; i<meatNames.size(); i++) { if (!rawMeats.contains(meatNames.get(i))) { List<ItemStack>
         * meatstacks = OreDictionary.getOres(meatNames.get(i)); if (!meatstacks.isEmpty()) { meatSortingList.add(new
         * FoodInfo(meatNames.get(i), meatstacks.size())); } } }
         */
        ...
        meatballRecipe = ExtremeCraftingManager.getInstance().addShapelessOreRecipe(
                new ItemStack(LudicrousItems.cosmic_meatballs, makesmeatballs),
                new ItemStack(LudicrousItems.resource, 1, 2));
    ...
    private static boolean bannedCrop(String crop) {
        for (String ban : forbiddenCropNames) {
            if (ban.equals(crop)) return true;
        }
        return false;
    }
