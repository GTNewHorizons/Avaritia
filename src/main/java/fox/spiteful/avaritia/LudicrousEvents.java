    ...
    public void onPlayerMine(PlayerInteractEvent event) {
        ...
        if (block.getBlockHardness(event.entityPlayer.worldObj, event.x, event.y, event.z) <= -1
            ...
            if (event.entityPlayer.getHeldItem().getTagCompound() != null
                    && event.entityPlayer.getHeldItem().getTagCompound().getBoolean("hammer")) {
                LudicrousItems.infinity_pickaxe.onBlockStartBreak(
                    ...
                    for (int i = 0; i < oreids.length; i++) {
                        ...
                        if (orename.startsWith("ore")) {
                            ...
                            adds.add(
                                    ifo.getStackForOre(
                                            drop,
                                            Math.min(drop.stackSize * (mult + 1), drop.getMaxStackSize())));
                            removals.add(drop);
                            break;
                        }
                    }
    ...
    private static boolean isGarbage(ItemStack drop) {
        for (int id : OreDictionary.getOreIDs(drop)) {
            for (String ore : trash) {
                if (OreDictionary.getOreName(id).equals(ore)) return true;
            }
        }
