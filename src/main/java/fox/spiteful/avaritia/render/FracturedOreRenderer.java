...
public class FracturedOreRenderer implements IItemRenderer {
        ...
        switch (type) {
            ...
            case INVENTORY: {
                ...
                if (item.hasTagCompound() && item.getTagCompound().hasKey(ItemFracturedOre.OREKEY)) {
                    ItemStack orestack = NameStack
                            .loadFromNBT(item.getTagCompound().getCompoundTag(ItemFracturedOre.OREKEY)).getStack();
                    ...
                    if (oreitem instanceof ItemBlock) {
                        /*
                         * ItemBlock ib = (ItemBlock)oreitem; Block b = ib.field_150939_a; IIcon oreicon = b.getIcon(0,
                         * orestack.getItemDamage()); float minu = oreicon.getMinU(); float maxu = oreicon.getMaxU();
                         * float minv = oreicon.getMinV(); float maxv = oreicon.getMaxV(); Tessellator t =
                         * Tessellator.instance; t.startDrawingQuads(); t.addVertexWithUV(0, 0, 0, minu, minv);
                         * t.addVertexWithUV(0, 16, 0, minu, maxv); t.addVertexWithUV(16, 16, 0, maxu, maxv);
                         * t.addVertexWithUV(16, 0, 0, maxu, minv); GL11.glDepthFunc(GL11.GL_EQUAL); t.draw();
    ...
    public void render(ItemStack item) {
        ...
        if (item.hasTagCompound() && item.getTagCompound().hasKey(ItemFracturedOre.OREKEY)) {
            ItemStack orestack = NameStack.loadFromNBT(item.getTagCompound().getCompoundTag(ItemFracturedOre.OREKEY))
                    .getStack();
