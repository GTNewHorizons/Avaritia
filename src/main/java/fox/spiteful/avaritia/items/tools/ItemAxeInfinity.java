/*
 * Code blatantly jacked from Vazkii Get the original here:
...
public class ItemAxeInfinity extends ItemAxe {
    ...
    private static BlockSwapper addBlockSwapper(World world, EntityPlayer player, ItemStack stack,
            ChunkCoordinates origCoords, ChunkCoordinates coords, int steps, boolean leaves, boolean force,
            List<String> posChecked) {
        BlockSwapper swapper = new BlockSwapper(
                world,
                player,
                stack,
                origCoords,
    ...
    private static class BlockSwapper {
        ...
        BlockSwapper(World world, EntityPlayer player, ItemStack stack, ChunkCoordinates origCoords,
                ChunkCoordinates coords, int steps, boolean leaves, boolean force, List<String> posChecked) {
            this.world = world;
            this.player = player;
            this.stack = stack;
        ...
        void tick() {
            ...
            if (!force && blockat.isAir(world, coords.posX, coords.posY, coords.posZ)) return;
            ToolHelper.removeBlockWithDrops(
                    player,
                    stack,
                    world,
                    coords.posX,
            ...
            for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (posChecked.contains(pstr)) continue;
