...
public class TonkersEvents {
    ...
    private void handleInfinityMods(NBTTagCompound toolTag, ToolCore tool) {
        ...
        if (toolTag.getInteger("Head") == Tonkers.infinityMetalId) modifiers += plusmod;
        if (toolTag.getInteger("Handle") == Tonkers.infinityMetalId) modifiers += plusmod;
        if (toolTag.getInteger("Accessory") == Tonkers.infinityMetalId) modifiers += plusmod;
        if (toolTag.getInteger("Extra") == Tonkers.infinityMetalId) modifiers += plusmod;

        // 2 part tools gain 2 modifiers for the head
        if (tool.getPartAmount() == 2 && toolTag.getInteger("Head") == Tonkers.infinityMetalId) modifiers += plusmod;
    ...
    public void onHurt(LivingHurtEvent event) {
        ...
        if (damager != null && damager instanceof EntityLivingBase && damaged instanceof EntityLivingBase) {
            ...
            if (held != null && held.hasTagCompound() && held.getItem() instanceof ToolCore) {
                ...
                float knockback = (AbilityHelper.calcKnockback(attacker, attacked, held, tool, toolTag, 0) + 1)
                        * puntpower;
                attacked.addVelocity(
                        (double) (-MathHelper.sin(attacker.rotationYaw * (float) Math.PI / 180.0F) * (float) knockback
                                * 0.5F),
                        0.1D,
                        (double) (MathHelper.cos(attacker.rotationYaw * (float) Math.PI / 180.0F) * (float) knockback
                                * 0.5F));
            }
        }
    }
